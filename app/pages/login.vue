...
async function login() {
  ...
  if (authErr || !authData.user) {
    loading.value = false
    message.value = authErr?.message || '登录失败'
    return
  }

  // Heal/verify admin access using session user (avoid stale useSupabaseUser)
  const { data: ok, error: accessErr } = await client.rpc('ensure_admin_access')
  if (accessErr || !ok) {
